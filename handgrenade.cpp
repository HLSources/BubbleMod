...
void CHandGrenade::Holster( int skiplocal /* = 0 */ )
{
	...
	if (m_flStartThrow)
	{
		...
		if (time < 0)
			time = 0;
		CGrenade *pGrenade = CGrenade::ShootTimed( m_pPlayer->pev, vecSrc, Vector(0,0,0), time );
		pGrenade->pev->classname = MAKE_STRING("hand_grenade");
		m_flStartThrow = 0;
		m_flReleaseThrow = 0;
	}
...
void CHandGrenade::WeaponIdle( void )
{
	...
	if ( m_flStartThrow )
	{
		...
		CGrenade *pGrenade = CGrenade::ShootTimed( m_pPlayer->pev, vecSrc, vecThrow, time );
		pGrenade->pev->classname = MAKE_STRING("hand_grenade");
